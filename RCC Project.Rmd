...
nndist <- c()
pair_correlation <- c()
pair_correlation_inhom <- c()
G_fun <- c()
hopskel_stat <- c()
rr <- data.frame(r=seq(20,50,by=0.2))
r <- c()
gamma <- c()
...
#pcf_inhom_ppp <- pcfinhom(split_process$Tumor,r = seq(0,50,0.1),correction = 'trans')
#pair_correlation_inhom <- rbind(pair_correlation_inhom,pcf_inhom_ppp$trans)
#G <- Kest(split_process$Tumor,r = seq(0,25,0.1),correction = "km")
#G_fun <- rbind(G_fun,G$km)
hopskel_stat <- c(hopskel_stat,hopskel(split_process$Tumor))
...
pcf_ppp <- pcf(split_process$Tumor,r = seq(0,50,by=0.1),correction = "trans",measure = "d")
pair_correlation <- rbind(pair_correlation,pcf_ppp$trans)
#pcf_inhom_ppp <- pcfinhom(split_process$Tumor,r = seq(0,50,0.1),measure = "b",correction = "trans")
#pair_correlation_inhom <- rbind(pair_correlation_inhom,pcf_inhom_ppp$trans)
#G <- Fest(split_process$Tumor,r = seq(0,25,0.1),correction="km")
#G_fun <- rbind(G_fun,G$km)
hopskel_stat <- c(hopskel_stat,hopskel(split_process$Tumor))
if(substr(geojson_files[file],1,12)%in% M0_id){
saveRDS(split_process$Tumor,file=paste("C:/Users/mathf/Downloads/No Metastasis/",geojson_files[file],".rds"))
